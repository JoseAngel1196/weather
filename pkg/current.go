...
func Current(cmd *cobra.Command, args []string) {
	...
	userLocation, err := internal.GetUserLocation()
	if err != nil {
		exitOnError("Sorry, looks like there was an error getting the user location", err)
	}

	// Fetch the weather data
	weatherData, err := internal.GetWeatherData(userLocation)
	if err != nil {
